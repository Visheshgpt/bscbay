...
export const getFeaturedPoolsData = () => {
  const featuredPoolData = poolsData.filter((item) => item.featured === true);
...
export const checkValue = (obj, type, id) => {
  if (obj && obj[type] && obj[type][id]) {
    return obj[type][id];
  }
  return null;
};

export const getPayload = (type, id, val) => {
  return {
    type,
    pay: {
      [id]: val,
