function HeaderButtons({userBNB, userBSC}) {
    ...
    <div className='btn_header'>
      <div className='btn_header_content px-3'>
        <div className='d-flex flex-column'>
          <div  style = { {fontSize:"12px"}} className='text-white'>{Number(bnbbalance).toFixed(2)} BNB</div>
          <div  style = { {fontSize:"12px"}} className=''>{(Number(bscbbalance)/1000000).toFixed(0)} Mn BSCB</div>  
        </div>
        <div className='b'>
          <a href='#'>
        ...
        <div className='icons'><img onClick={logoutUser} src="/assets/logout3.png"></img></div>
      </div>
      <div style={ {color:"white"} } className='date'>Feb 11, 08.06 UTC</div>
    </div>
  );
}
